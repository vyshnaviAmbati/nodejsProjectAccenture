...
test.after(async () => {
  ...
  await fs.rm(path.join(processedDirectory, 'test-image-8x6.jpg'), {
    force: true,
  });
});
test('resizes a requested source image', async () => {
  const response = await request(app).get(
    '/api/images?filename=test-image.jpg&width=8&height=6',
  );
...
test('returns 404 for a missing source image', async () => {
  const response = await request(app).get(
    '/api/images?filename=missing.jpg&width=120&height=80',
  );
...
test('rejects missing or invalid dimensions', async () => {
  const response = await request(app).get(
    '/api/images?filename=mountain.jpg&width=wide&height=0',
  );
...
test('rejects path traversal and unsupported files', async () => {
  const response = await request(app).get(
    '/api/images?filename=../secret.txt&width=100&height=100',
  );
  assert.equal(response.status, 400);
